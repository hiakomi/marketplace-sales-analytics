-- Взвешенная retention-кривая
-- Период исследования: 2023 год. Запускать отдельно.
--Ретеншн - кривая по когортам
...
SELECT
    r.month_number,
    SUM(r.active_clients) AS active_clients,
    SUM(c.cohort_size) AS customers_at_start,
    ROUND(
        100.0 * SUM(r.active_clients) / SUM(c.cohort_size),
        2
    ) AS retention_pct
FROM retention r
JOIN cohort_sizes c
